...
import funcs
from forms.searcherInfo import searcher
import math
from tkinter import *
...
class App(tk.Tk):
    ...
    def getTab1(self,tab_control):
        ...
        tab_control.add(tab2, text='Фермерская база')
    ...
    def getTab2(self,tab_control):
        ...
        tab_control.add(tab, text='Поиск рынка')
        ...
        ttk.Button(tab, text="Поиск", command=lambda: self.search(False,self.tableSearcher)).grid(row=2, column=2,sticky=W)

    def getTab3(self,tab_control):
        tab = ttk.Frame(tab_control)
        tab.rowconfigure(1, weight=15)
        tab_control.add(tab, text='Поиск рынка по x/y')
        ...
        self.tableCoordsSearcher = ttk.Treeview(tab, show="headings", selectmode="browse")

        xscrollbar = ttk.Scrollbar(tab, orient=HORIZONTAL, command=self.tableCoordsSearcher.xview)
        xscrollbar.grid(row=5, column=0, columnspan=4, sticky=EW)
        self.tableCoordsSearcher.configure(xscrollcommand=xscrollbar.set)
        tab.grid_columnconfigure(0, weight=1)
        tab.grid_rowconfigure(0, weight=1)
        self.tableCoordsSearcher.config(height=20)
        self.tableCoordsSearcher.grid( row=4,column=0, columnspan=4, sticky=NSEW)
        ttk.Button(tab, text="Поиск", command=lambda:self.search(True,self.tableCoordsSearcher)).grid(row=2, column=2,sticky=W)
    ...
    def search(self,status,tree):
        ...
        if(status == False):
            ...
            rows = df.values
            headings = list(df.columns)
            tree.bind('<Double-Button-1>', self.selectSearchTable)
            ...
            if(self.findEntryY.get() != "" and self.findEntryX.get() != "" and self.findEntryZ.get() != ""):
                try:
                    ...
                    tree["columns"] = headings
                    tree["displaycolumns"] = headings
                    tree.bind('<Double-Button-1>', self.selectSearchTableCoords)
                except:
                    print("ERROR")
                    return
    ...
    def selectSearchTableCoords(self, event):
        curitem = self.tableCoordsSearcher.focus()
        item = self.tableCoordsSearcher.item(curitem)
        listMarket = list(item.values())[2]
        searcherInfo = searcher([listMarket[0], listMarket[1]])
